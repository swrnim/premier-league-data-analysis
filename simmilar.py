...
def sim(plrdata):
	...
	print("\nMost Similar Player: ")
	for player, distance, stat_similarities in similarities[:1]:
		simper = (
			1 - distance/maxdistance
		) * 100
		print(f"\n{player}, ({simper:.2f}% similar)")
		print("Most Similar Stats:")
		statname = []
		for stat, stat_sim_pct in stat_similarities[:8]:
			clean_stat = stat.replace("_percentile", "")
			print(f"- {clean_stat}: ({round(stat_sim_pct, 1)}% similar)")
			statname.append(stat)

		plrdata2= get_profile(profiles_df[profiles_df["player_name"]==player])
		graph.spidergraph(chplyr, plrdata2, statname)
